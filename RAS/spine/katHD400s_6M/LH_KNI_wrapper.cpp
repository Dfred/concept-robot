...
sendSplineToMotor(int axis, int targetpos, int duration,
  ...
  try{
    katana->sendSplineToMotor((unsigned short) (axis-1), (short) targetpos,
...
getPosition(struct TPos *pos){
  try{
    katana->getCoordinates(pos->X, pos->Y, pos->Z,
			   pos->phi, pos->theta, pos->psi);
  }
  catch(Exception &e){
    std::cout << "ERROR: " << e.message() << std::endl;
    ERROR(e); return ERR_FAILED;
...
moveToPosLin(struct TPos *targetPos, int velocity, int acceleration){
  try{
    setMaxAccel(0, acceleration);
    setMaxVelocity(0, velocity);
    katana->moveRobotLinearTo(targetPos->X, targetPos->Y, targetPos->Z,
			      targetPos->phi, targetPos->theta, targetPos->psi);
  }
  catch(Exception &e){
    std::cout << "ERROR: " << e.message() << std::endl;
    ERROR(e); return ERR_FAILED;
...
executeMovement(struct TMovement *movement){
  ...
  if (movement->transition == PTP) {
    //move PTP
    status = moveToPos(&(movement->pos), movement->velocity,
		       movement->acceleration);
  } else if (movement->transition == LINEAR) {
    //move linear
    status = moveToPosLin(&(movement->pos), movement->velocity,
			  movement->acceleration);
  } else {
    status = ERR_FAILED;
  }
