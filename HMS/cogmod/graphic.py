...
class GUI(QMainWindow):
    """ main window
    """
    def __init__(self, from_gui_q=None, from_beh_q=None):
        ...
        self.mouse_x = 0
        self.mouse_y = 0
        self.cam_resolution = (2, (960,544)) 	
        #self.cam_resolution = (1, (640,480))
        haar_casc = cfg.root_dir_dev + "resources/haarcascade_frontalface_alt.xml"        # change path for compatibility
    ...
    def set_defaults(self):
        ...
        self.attention = cv.CreateImage(self.cam_resolution[1], 8, 1)    # create attention heatmap
        cv.SetZero(self.attention)
        self.blank = cv.CreateImage(self.cam_resolution[1], 8, 1)        # create blank image
        cv.Set(self.blank, 2)
        self.saliency = cv.CreateImage(self.cam_resolution[1], 8, 1)     # saliency map
        cv.Set(self.saliency, 255)
        self.hsv_saliency_map = cv.CreateImage( self.cam_resolution[1], 8, 3 )
        ...
        # for movement2
        self.motion_history = np.zeros((self.cam_resolution[1][1], self.cam_resolution[1][0]), np.float32)
        self.hsv = np.zeros((self.cam_resolution[1][0], self.cam_resolution[1][1], 3), np.uint8)
        self.hsv[:,:,1] = 255
    ...
    def robot_follow_target(self):
        ...
        # there is a target
            ...
            if gl.from_gui_q:   # there is a connection
                if self.layout.checkBox_70.isChecked():
                    tuning = (gl.gaze_tune_x, gl.gaze_tune_y,  gl.neck_tune_x, gl.neck_tune_y)
                    gl.from_gui_q.put(("face_gaze", self.robot_target, tuning), None)    # make first face available
                if self.layout.checkBox_71.isChecked():
                    tuning = (gl.gaze_tune_x, gl.gaze_tune_y,  gl.neck_tune_x, gl.neck_tune_y)
...
################### Language Games ###################
    ...
    def start_camera(self):
        if not self.camera_running:
            ...
            self.webcam = Webcam(self.cam_resolution[0], self.cam_resolution[1])
            
            try:
                self.webcam.query()
                self.camera_running = True
                self.layout.label.show()
                self.layout.pushButton_2.setText("Stop camera")
            except AttributeError:
                print "No camera found, please make sure a camera is connected"
                self.statusBar().showMessage("No camera found, please make sure a camera is connected")

            while self.camera_running:
                im = self.webcam.query()
                #cv.SaveImage("test.png", im.asOpenCV())
                ...
                if self.layout.checkBox.isChecked():
                    self.detect_face(im)
                ...
                if self.layout.checkBox_2.isChecked():
                    im = canny(im, (self.layout.horizontalSlider.value() * 1.0), (self.layout.horizontalSlider.value() * 3.0))
                    self.statusBar().showMessage("Detecting edges")
                ...
                self.layout.label.setPixmap(qpixmap)          # update label pixmap

                fps = self.get_fps()
                if fps:
                    self.layout.label_16.setText("FPS: " + str(fps))
                    
                # general display of information    
                self.layout.textEdit.setPlainText(self.info)
                
                if self.current_agent is not None and self.current_sensory_input is not None and self.layout.checkBox_16.isChecked():
                    self.current_object_name = self.current_agent.name_object([["hri", self.current_sensory_input]])
                else: 
                    self.current_object_name = None
                
                #update robot behaviour status
                self.update_behaviour_status()
        ...
        if self.camera_running:
            self.webcam = None
            self.camera_running = False
            self.layout.label.hide()
            self.layout.pushButton_2.setText("Start camera")
    ...
    def detect_face(self, img):
        ...
        if faces:
            ...
            if self.layout.checkBox_25.isChecked(): #histogram checking
        ...
        self.info += str(len(faces)) + " face(s) detected\n"
...
#            self.info += "\n Histogram similarity: \n" + str(compare) + "\n\n"
    ...
    def set_threshold(self, im):
        im_threshold = cv.CreateImage( self.cam_resolution[1], 8, 1 )
    ...
    def set_adaptive_threshold(self, im):
        thresh_image = cv.CreateImage( self.cam_resolution[1], 8, 1 )
    ...
    def detect_circle(self, image):
        ...
        grayscale = cv.CreateImage(self.cam_resolution[1], 8, 1)
        grayscale_smooth = cv.CreateImage(self.cam_resolution[1], 8, 1)
    ...
    def detect_movement(self, im):
        ...
        dst = cv.CreateImage(self.cam_resolution[1], 8, 1)
...
#            cv.Circle(dst, (int(act_coors[0]*width + 0.5*width), int(act_coors[1]*height + 0.5*height)), 50, (255, 255, 255))   # draw in the original image
    ...
    def detect_movement2(self, im):
        ...
        for i, rect in enumerate([(0, 0, self.cam_resolution[1][0], self.cam_resolution[1][1])] + list(seg_bounds)):
            x, y, rw, rh = rect
            area = rw*rh
            if area < 64**2:
                continue
    ...
    def update_saliency(self, im):
        ...
        saliency_copy = cv.CreateImage( self.cam_resolution[1], 8, 1 )
        ...
        bgr_copy = cv.CreateImage( self.cam_resolution[1], 8, 3 )  
